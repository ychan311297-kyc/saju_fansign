...
data.forEach((record, index) => {
  ...
  required.forEach((key) => {
    if (!(key in record)) {
      fail(`record ${row} is missing "${key}"`);
    }
  ...
  if (!record.name.trim()) {
    fail(`record ${row} has an empty name`);
  }

  if (!record.group.trim()) {
    fail(`record ${row} has an empty group`);
  }

  if (!record.agency.trim()) {
    fail(`record ${row} has an empty agency`);
  }

  if (!record.cat.trim()) {
    fail(`record ${row} has an empty cat`);
  }
  ...
  const duplicateKey =
    `${record.name.trim().toLowerCase()}::${record.group.trim().toLowerCase()}`;
